"""Silver flow: IBGE PAS — table 2715 (services indicators by region/CNAE)."""
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df[list(BrIbgePasTbl2715.model_fields.keys())]
...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    [BrIbgePasTbl2715(**r) for r in df.to_dict("records")]
    return df


def load(df: pd.DataFrame) -> None:
    columns = pydantic_to_postgres_columns(BrIbgePasTbl2715)
    with PostgresETL(
        host="localhost",
