"""Silver flow: Censo Agropecuário 2006 — table 2233 (autoconsumo by product)."""
...
from dados.silver.models.al_ibge_censoagro import AlIbgeCensoagroTbl22332006
from dados.silver.padronizacao_produtos import dicionario_produtos_censo_6949_2233
...
def extract() -> pd.DataFrame:
    query = f"""
        SELECT
            ...
            MAX(CASE WHEN nome_variavel = 'Quantidade colhida' THEN valor END) AS quantidade_produzida,
            ...
            MAX(CASE WHEN nome_variavel = 'Valor das vendas' THEN valor END) AS valor_venda
        FROM {DATASET_ID}.{TABLE}
        WHERE tipo_agricultura IN ('Agricultura familiar - Lei 11.326', 'Agricultura não familiar')
        GROUP BY ano, id_municipio, produto, tipo_agricultura, tipo_consumo_estocagem
    """
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = fix_ibge_digits(
        df,
        METRIC_COLS,
        ["id_municipio", "ano", "produto", "tipo_agricultura", "tipo_consumo_estocagem"],
    ...
    return df[list(AlIbgeCensoagroTbl22332006.model_fields.keys())]
...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    [AlIbgeCensoagroTbl22332006(**r) for r in df.to_dict("records")]
    return df
...
def flow() -> None:
    ...
    try:
        ...
        write_silver(TABLE, df, AlIbgeCensoagroTbl22332006); log.info("load.done", rows=len(df))
