"""Silver flow: Censo Agropecuário 2017 — table 6898 (production value by type)."""
...
from dados.silver.models.al_ibge_censoagro import AlIbgeCensoagroTbl68982017
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = fix_ibge_digits(
        df,
        METRIC_COLS,
        ["id_municipio", "ano", "tipo_agricultura"],
        div_column="quantidade_estabelecimentos_produtivos",
    )
    return df[list(AlIbgeCensoagroTbl68982017.model_fields.keys())]
...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    [AlIbgeCensoagroTbl68982017(**r) for r in df.to_dict("records")]
    return df
...
def flow() -> None:
    ...
    try:
        ...
        write_silver(TABLE, df, AlIbgeCensoagroTbl68982017); log.info("load.done", rows=len(df))
