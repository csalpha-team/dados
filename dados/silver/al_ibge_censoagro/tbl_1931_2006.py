"""Silver flow: Censo Agropecuário 2006 — table 1931 (production value by type)."""
...
from dados.silver.models.al_ibge_censoagro import AlIbgeCensoagroTbl19312006
...
TABLE = "tbl_1931_2006"
PK_COLS = ["ano", "id_municipio", "tipo_agricultura", "tipo_producao"]
...
def extract() -> pd.DataFrame:
    query = f"""
        ...
        GROUP BY ano, id_municipio, tipo_agricultura, tipo_producao
    """
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = fix_ibge_digits(
        df,
        METRIC_COLS,
        ["id_municipio", "ano", "tipo_agricultura", "tipo_producao"],
        div_column="quantidade_estabelecimentos_produtivos",
    )
    return df[list(AlIbgeCensoagroTbl19312006.model_fields.keys())]
...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    [AlIbgeCensoagroTbl19312006(**r) for r in df.to_dict("records")]
    return df
...
def flow() -> None:
    ...
    try:
        ...
        write_silver(TABLE, df, AlIbgeCensoagroTbl19312006); log.info("load.done", rows=len(df))
