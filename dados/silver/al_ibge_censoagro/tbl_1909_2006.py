"""Silver flow: Censo Agropecuário 2006 — table 1909 (farm expenditures)."""
...
from dados.silver.models.al_ibge_censoagro import AlIbgeCensoagroTbl19092006
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df[list(AlIbgeCensoagroTbl19092006.model_fields.keys())]
...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    [AlIbgeCensoagroTbl19092006(**r) for r in df.to_dict("records")]
    return df
...
def flow() -> None:
    ...
    try:
        ...
        write_silver(TABLE, df, AlIbgeCensoagroTbl19092006); log.info("load.done", rows=len(df))
