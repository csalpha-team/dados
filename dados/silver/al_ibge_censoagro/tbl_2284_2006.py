"""Silver flow: Censo Agropecuário 2006 — table 2284 (autoconsumo by product, no valor_venda)."""
...
from dados.silver.models.al_ibge_censoagro import AlIbgeCensoagroTbl22842006
from dados.silver.padronizacao_produtos import dicionario_produtos_censo_6957_2337
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df[list(AlIbgeCensoagroTbl22842006.model_fields.keys())]
...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    [AlIbgeCensoagroTbl22842006(**r) for r in df.to_dict("records")]
    return df
...
def flow() -> None:
    ...
    try:
        ...
        write_silver(TABLE, df, AlIbgeCensoagroTbl22842006); log.info("load.done", rows=len(df))
