...
def construir_coeficientes_consumo(
    ...
    if coluna_tipo_despesa_mip not in mip_mapping_df.columns:
        raise ValueError(f"Coluna '{coluna_tipo_despesa_mip}' ausente no de-para do MIP.")
    ...
    pof_merged = pof_df.merge(
        ...
        how="inner",
    ).drop(columns=[coluna_tipo_despesa_mip, "unidade_medida", "Unnamed: 0"], errors="ignore")
    ...
    filtrado = pof_filtrada[mascara_urbana | mascara_rural].copy().reset_index(drop=True)

    filtrado = filtrado.dropna(subset=["coeff_key", "valor"])
    filtrado = filtrado.drop_duplicates(subset=["coeff_key"], keep="last")

    coefficients = filtrado[["ano", "coeff_key", "valor"]].rename(columns={"valor": "coeff"})
